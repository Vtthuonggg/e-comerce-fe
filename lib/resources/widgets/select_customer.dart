...
class CustomerSelect extends NyStatefulWidget {
  final GlobalKey<DropdownSearchState<Customer>> selectKey;
  ...
  CustomerSelect({
    ...
    this.selectedItem,
    this.labelText = 'Chọn khách hàng',
    this.hintText = 'Chọn khách hàng',
  }) : super(key: key);

  @override
  NyState<CustomerSelect> createState() => _CustomerSelectState();
}

class _CustomerSelectState extends NyState<CustomerSelect> {
  static const _pageSize = 10;
  final PagingController<int, Customer> _pagingController =
      PagingController(firstPageKey: 1);
  String keyword = '';
  TextEditingController searchBoxController = TextEditingController();
  Customer? selectedItemTmp;
  Timer? _debounce;
  ...
  _fetchPage(int pageKey) async {
    try {
      var res = await api<CustomerApiService>((request) => request.listCustomer(
          keyword.isEmpty ? null : keyword, pageKey, _pageSize));
      List<Customer> customerItems =
          res['data'].map<Customer>((data) => Customer.fromJson(data)).toList();

      final isLastPage = customerItems.length < _pageSize;
      if (isLastPage) {
        _pagingController.appendLastPage(customerItems);
      } else {
        final nextPageKey = pageKey + 1;
        _pagingController.appendPage(customerItems, nextPageKey);
      }
    } catch (error) {
  ...
  Widget build(BuildContext context) {
    ...
    return DropdownSearch<Customer>(
                  ...
                  children: [
                    Align(
                      ...
                      child: Container(
                        ...
                        child: Text(
                          widget.labelText ?? 'Chọn khách hàng',
                      ...
                      Expanded(
                        ...
                        child: Row(
                          ...
                          children: [
                            InkWell(
                              onTap: () {
                                routeTo(EditCustomerPage.path, onPop: (value) {
                                  _pagingController.refresh();
                                });
                              },
                              child: Container(
      ...
      onChanged: (Customer? value) {
        // This is triggered when clicking on popup items
      },
  ...
  Widget buildPopupItem(BuildContext context, Customer item) {
