...
class ToppingMultiSelect extends NyStatefulWidget {
  final GlobalKey<DropdownSearchState<Product>> multiKey;
  final void Function(List<Product>? toppings) onSelect;
  final List<Product> selectedToppings;
  ...
  @override
  NyState<ToppingMultiSelect> createState() => ToppingMultiSelectState();
}

class ToppingMultiSelectState extends NyState<ToppingMultiSelect> {
  ...
  _fetchPage(int pageKey) async {
    try {
      var res = await api<ProductApiService>((request) => request.listTopping(
      ...
      listItemsSelectedTmp = [...widget.selectedToppings];
                      ...
                      child: PagedGridView<int, Product>(
                        ...
                        builderDelegate: PagedChildBuilderDelegate<Product>(
                          ...
                          noItemsFoundIndicatorBuilder: (_) =>
                              Center(child: Text("Không tìm thấy topping nào")),
                        ),
                      ),
                    ),
          ...
          Expanded(
            child: ElevatedButton(
              ...
              onPressed: () {
                keyword = '';
                Navigator.pop(context);
                addMultiToppings(listItemsSelectedTmp);
                setState(() {
  ...
  void addMultiToppings(List<Product> items) {
    if (items.isEmpty) {
      widget.selectedToppings.clear();
      widget.onSelect(widget.selectedToppings);
      return;
    }

    for (var item in items) {
      if (widget.selectedToppings
              .indexWhere((element) => element.id == item.id) ==
          -1) {
        // Initialize new topping
        item.quantity = 1;
        item.txtQuantity.text = '1';
        setState(() {
          widget.selectedToppings.add(item);
        });
      }
    }

    widget.selectedToppings.removeWhere((i) {
